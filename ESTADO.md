...
⏸️ CHECKPOINT — Última acción completada (01/09/2026): **Resend conectado como SMTP de Supabase** (sin dominio propio aún — usuario no tiene presupuesto ahora mismo, se usa el remitente de pruebas `onboarding@resend.dev`, que solo entrega al correo del dueño de la cuenta de Resend, suficiente para seguir probando; el límite subió de 2/hora a 30, confirmado en logs). También se corrigió un bug real de configuración: la "Site URL" de Supabase Auth apuntaba a `localhost:3000` desde la creación del proyecto — los magic links en producción redirigían a la máquina del usuario en vez de `gymevo-eta.vercel.app`. Corregido en el dashboard de Supabase (Site URL + Redirect URLs). Login real confirmado funcionando de punta a punta después de ambas correcciones. Además se agregó: video de fondo también en "Plan de hoy" (antes solo en la landing) y nombre de usuario editable en Perfil ("Hola, {nombre}", persistido en Supabase). / Siguiente acción exacta: seguir con el pulido pendiente de Sesión 7 (landing y onboarding no pasan el gate del revisor-visual) o con dominio propio → Hotmart cuando el usuario tenga presupuesto.
