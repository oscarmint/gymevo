...
- **veredicto:paywall** — YA EXISTE (docs/revisiones/paywall-veredicto.md, subagente `revisor-visual`, Sesión 7): NO LISTA (Usabilidad 30/40, Craft 12/20, Copy 18/20). Defectos concretos documentados; pendiente aplicar el pulido antes de re-lanzar el revisor y declarar lista.
- **veredicto:landing** — YA EXISTE pero la primera pasada quedó invalidada por un bug del script de captura (screenshot casi en blanco, corregido — ver sección "Sesión 7" arriba); se relanzó el revisor con la captura corregida, resultado pendiente de esta misma sesión.
- **veredicto:onboarding** — YA EXISTE (docs/revisiones/onboarding-veredicto.md, Sesión 7): NO LISTA (Usabilidad 31/40, Craft 11/20). Defectos concretos documentados; pendiente aplicar el pulido antes de re-lanzar el revisor.
- **veredicto:pantalla-principal** — mismo caso que landing: primera pasada con datos de ejemplo poco representativos, recapturada, resultado pendiente.
- Las 4 pantallas del dinero (landing, paywall, onboarding, pantalla-principal) están en pulido activo de Sesión 7: se corrigen los defectos reales que encontró el revisor y se vuelve a lanzar hasta que las 4 pasen el gate (≥36/40 usabilidad, ≥16/20 craft, ≥16/20 copy en las que venden). No se declaran "listas" hasta que el veredicto lo diga explícitamente — autoevaluarse no cuenta como evidencia (Regla de Oro 7).
